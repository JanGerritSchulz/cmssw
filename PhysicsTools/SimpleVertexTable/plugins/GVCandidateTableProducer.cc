...
void GenVertexCandidateProducer::produce(edm::Event& iEvent, const edm::EventSetup&) {
  ...
  for (size_t i = 0; i < genParticles->size(); ++i) {
    ...
    for (size_t j = 0; j < genParticles->size(); ++j) {
      ...
      if (dau == hadron)
        continue;
      if (!(dau->status() == 1 && dau->charge() != 0 && dau->pt() > 0.8 && std::abs(dau->eta()) < 2.5))
        continue;
  ...
  auto result = matchHadronsToSV(distances,
                                 SVtrk_pt,
                                 SVtrk_eta,
                                 SVtrk_phi,
                                 SVtrk_SVidx,
...
//  isAncestor()
std::optional<std::tuple<float, float, float>> GenVertexCandidateProducer::isAncestor(
    const reco::Candidate* ancestor, const reco::Candidate* particle) const
...
std::vector<std::vector<float>> GenVertexCandidateProducer::computeDistanceMatrix(const std::vector<float>& SV_x,
                                                                                  const std::vector<float>& SV_y,
                                                                                  const std::vector<float>& SV_z,
                                                                                  const std::vector<float>& Hadron_GVx,
                                                                                  const std::vector<float>& Hadron_GVy,
...
void GenVertexCandidateProducer::printDistanceMatrix(const std::vector<std::vector<float>>& distances) {
  ...
  while (true) {
      ...
      for (size_t iHad : GenDaughtersIdxs_fromBestHad) {
        ...
        if (dR < dR_max && relPt < relPt_max) {
          ...
          if (common >= nRequiredCommonTracks)
            break;  // break the iHad cycle
        }
      }
      if (common >= nRequiredCommonTracks)
        break;  // break the iSV cycle
    }
    ...
    if (common >= nRequiredCommonTracks) {
      ...
      for (int h = 0; h < n_Hadrons; ++h)
        distances[bestSV][h] = 998.0;  // remove SV row
      for (size_t s = 0; s < nSV; ++s)
        distances[s][bestHad] = 998.0;  // remove Hadron column
