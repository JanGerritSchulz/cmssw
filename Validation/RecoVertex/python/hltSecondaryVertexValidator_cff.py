...
from Validation.RecoVertex.secondaryVertexAnalyzer_cfi import *
...
hltSecondaryVertexValidator = secondaryVertexAnalyzer.clone(
    ...
    recoVertexCollections = cms.VInputTag(["hltDeepInclusiveMergedVertices"]),
...
hltSecondaryVertexValidatorCPC = secondaryVertexAnalyzerCPC.clone(
    rootFolder = cms.untracked.string('HLT/SecondaryVertices/Validation'),
    verbose = cms.untracked.bool(False),
    doGenericSimPlots = cms.untracked.bool(True),
    doPerPdgPlots = cms.untracked.bool(True),
    recoVertexCollections = cms.VInputTag(["hltDeepInclusiveMergedVerticesPF"]),
    vertexAssociators = cms.VInputTag(["hltSVAssociatorByPositionAndTracks4GeneralTracksCPC"]),
    primaryVertices = cms.InputTag('hltOfflinePrimaryVertices'),
    hepMCProduct = cms.InputTag('generatorSmeared'),
    simVertices = cms.InputTag('mix', 'MergedTrackTruth'),
    trackAssociation = cms.InputTag('tpToHLTGeneralTrackAssociation'),
...
hltSecondaryVertexValidation = cms.Sequence(
    hltSecondaryVertexValidatorCPC,
    hltSVAssociationsTask
)


_hltSecondaryVertexValidationNGTSVs = cms.Sequence(
...
ngtSVs.toReplaceWith(hltSecondaryVertexValidation, _hltSecondaryVertexValidationNGTSVs)
