...
private:
  ...
  std::vector<float> trackingParticle_x_;
  std::vector<float> trackingParticle_y_;
  std::vector<float> trackingParticle_z_;
  std::vector<float> trackingParticle_charge_;
...
void PixelTrackNtuplizer::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  // selections of what to save
  desc.add<bool>("storeTrackingParticles", true);
...
PixelTrackNtuplizer::PixelTrackNtuplizer(const edm::ParameterSet& pSet)
    : topology_getToken_(esConsumes<TrackerTopology, TrackerTopologyRcd>()),
      ...
      storeTPs_(pSet.getParameter<bool>("storeTrackingParticles")) {
...
void PixelTrackNtuplizer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  for (const TrackingParticleRef& tp : tpCollection) {
    ...
    if (storeTPs_) {
      ...
      trackingParticle_x_.push_back(tp->vx());
      trackingParticle_y_.push_back(tp->vy());
      trackingParticle_z_.push_back(tp->vz());
...
void PixelTrackNtuplizer::beginJob() {
  ...
  if (storeTPs_) {
    ...
    output_tree_->Branch("trackingParticle_x", &trackingParticle_x_);
    output_tree_->Branch("trackingParticle_y", &trackingParticle_y_);
    output_tree_->Branch("trackingParticle_z", &trackingParticle_z_);
    output_tree_->Branch("trackingParticle_charge", &trackingParticle_charge_);
...
void PixelTrackNtuplizer::clearVectors() {
  ...
  trackingParticle_x_.clear();
  trackingParticle_y_.clear();
  trackingParticle_z_.clear();
